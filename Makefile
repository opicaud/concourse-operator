...
# trackclear.be/concourse-operator-bundle:$VERSION and trackclear.be/concourse-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= docker-registry.test/concourse-operator
...
run: ansible-operator ## Run against the configured Kubernetes cluster in ~/.kube/config
	ANSIBLE_ROLES_PATH="$(pwd)/roles" $(ANSIBLE_OPERATOR) run
...
uninstall: kustomize ## Uninstall CRDs from the K8s cluster specified in ~/.kube/config.
	$(KUSTOMIZE) build config/crd | minikube kubectl -- delete -f -
